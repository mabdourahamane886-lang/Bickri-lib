...
export async function getBooks(query = "", level = "", category = "", subject = ""): Promise<Book[]> {
  ...
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY) {
    const filtered = demoBooks.filter(book => {
      const matchesQuery = !normalized || `${book.title} ${book.author ?? ""} ${book.description ?? ""}`.toLowerCase().includes(normalized.toLowerCase());
      const matchesLevel = !normalizedLevel || book.level === normalizedLevel;
      const matchesCategory = !normalizedCategory || book.category === normalizedCategory;
      const matchesSubject = !normalizedSubject || book.subject === normalizedSubject;
      return matchesQuery && matchesLevel && matchesCategory && matchesSubject;
    });
    return filtered;
  }
  ...
  try {
    ...
    let request = supabase
      .from("books")
      .select("id,title,author,description,cover_url,level,subject,category,language,is_premium,published_year,created_at")
      .order("created_at", { ascending: false });

    if (normalized) {
      const safeQuery = normalized.replace(/[%_,]/g, " ").trim();
      request = request.or(`title.ilike.%${safeQuery}%,author.ilike.%${safeQuery}%,description.ilike.%${safeQuery}%,subject.ilike.%${safeQuery}%`);
    }
