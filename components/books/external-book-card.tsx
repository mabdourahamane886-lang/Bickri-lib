...
const sourceLabels: Record<ExternalBook['source'], string> = {
...
export default function ExternalBookCard({ book }: { book: ExternalBook }) {
  ...
  const accessLabel = book.access_type === 'full' ? 'Lecture disponible' : book.access_type === 'preview' ? 'Aperçu' : 'Voir la source';
  ...
  return (
    <article className="book-card">
      <div className="cover">
        ...
        <span className="badge"><Globe2 size={11} /> {sourceLabels[book.source]}</span>
      </div>
