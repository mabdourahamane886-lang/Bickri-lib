...
export default async function BooksPage({ searchParams }: { searchParams: Promise<{ q?: string; level?: string; category?: string; subject?: string }> }) {
  ...
  return <div className="shell"><Sidebar/><MobileNavigation/><main className="page-main"><div className="container">
    ...
    {books.length ? <div className="book-grid">{books.map(book=><BookCard key={book.id} book={book}/>)}</div> : <div className="card" style={{textAlign:"center",padding:"40px 20px"}}><h2 style={{color:"#071a33"}}>Aucune ressource locale</h2><p className="muted">Des ressources complémentaires peuvent quand même proposer des résultats ci-dessous.</p></div>}

    {externalBooks.length > 0 && <section className="section"><div className="section-head"><div><p className="eyebrow" style={{color:"#9a6b0e",margin:0}}>Recherche complémentaire</p><h2><Globe2 size={20} style={{verticalAlign:"-3px",marginRight:7}}/>Ressources complémentaires</h2><p className="muted" style={{margin:"5px 0 0"}}>{externalBooks.length} résultats issus de catalogues publics et académiques.</p></div></div><div className="book-grid">{externalBooks.map(book=><ExternalBookCard key={book.id} book={book}/>)}</div></section>}

    <div className="card section" style={{background:"#071a33",color:"white"}}><h2 style={{margin:"0 0 8px"}}>Ressources intégrées</h2><p style={{margin:0,color:"#cbd5e1",lineHeight:1.6}}>Bickri Lib combine son catalogue local avec des ressources complémentaires pour élargir les possibilités de recherche et de lecture.</p></div>
