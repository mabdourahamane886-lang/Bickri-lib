...
const features=[
  ...
  [ShieldCheck,"Accès sécurisé","Votre compte et vos données sont protégés par Supabase Auth."],
];
export default function HomePage(){return <div className="shell"><Sidebar/><MobileNavigation/><main className="page-main"><div className="container">
  ...
  <section className="section"><div className="section-head"><div><p className="eyebrow" style={{color:"#9a6b0e",margin:0}}>Pourquoi Bickri Lib</p><h2>Une expérience pensée pour apprendre</h2></div></div><div className="grid grid-3">{features.map(([Icon,title,text])=>{const I=Icon as typeof Sparkles;return <div className="card feature" key={title as string}><div className="feature-icon"><I size={20}/></div><div><h3 style={{margin:"0 0 7px",color:"#071a33"}}>{title as string}</h3><p className="muted" style={{margin:0,lineHeight:1.6,fontSize:14}}>{text as string}</p></div></div>})}</div></section>
